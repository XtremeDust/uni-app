...
export default function AsideMobile({onNavigate, CurrentKey, ...props}: AsideProps){
    ...
    const handleOptions = () => setOptionOpen(!isOptionOpen);
    ...
    const isActive = invisibleOptions.some(item => {
        if(item.id === CurrentKey) return true;
        if(item.submenu) return item.submenu.some(sub => sub.id === CurrentKey);
        return false;
    });
                    ...
                    <div className="bg-white absolute bottom-0 h-auto w-full shadow-2xl p-4 flex flex-col gap-1.5" onClick={(e) => e.stopPropagation()}>
                        ...
                        {invisibleOptions.map((item) => (
                            <div key={item.id}>
                                <div
                                    onClick={(e) => { 
                                        if(!item.submenu){ handleMenuClick(item.id, !!item.submenu); }
                                        else{ e.stopPropagation(); handleMenuClick(item.id, !!item.submenu); }
                                    }}
                                ...
                                {item.submenu && item.id === openSubmenu && (
                                    <div className={`ml-3 pl-2 pr-2 border-l border-gray-300 mt-1.5 gap-1 flex flex-col`}>
                                        {item.submenu.map((sub) => (
                                            ...
                                            >
                                                <Image className='size-8' src={sub.img} alt={sub.section} width={100} height={100} />
                                                <h3>{sub.section}</h3>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className=' border-t pt-1.5'>
                            <Button 
                                ...
                                className='w-full p-3 flex gap-3 items-center bg-red-200/50 hover:bg-red-200/75 text-red-700 cursor-pointer text-start rounded-lg '
                            >
                                <Image className='size-8' src={'/cerrar-sesion.png'} alt={'cerrar'} width={100} height={100} />                            
                                Cerrar Sesión
                            </Button>
                        </div>
                    </div>
