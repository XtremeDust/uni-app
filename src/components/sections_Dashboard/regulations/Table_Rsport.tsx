...
import { 
  Input, Button, Select, Table, TableBody, TableCell, TableRow, TableHead, TableHeaderCell
...
export default function Tabla_Rsport() {
  ...
  const [editingReg, setEditingReg] = useState<ApiSportRegulation | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);
  const [regToDelete, setRegToDelete] = useState<ApiSportRegulation | null>(null);
  ...
  // --- Funciones ---
  const handleEditClick = (reg: ApiSportRegulation) => {
      console.log("Editando:", reg);
      setEditingReg(reg);
  };

  const handleDeleteClick = (reg: ApiSportRegulation) => {
      console.log("Eliminando:", reg);
      setRegToDelete(reg);
  };
              ...
              {sportRegs.map((data)=>(
                ...
                >
                  ...
                  <TableCell className="space-x-2 flex justify-evenly text-white">
                    {buttons.map((btn)=>(            
                        btn.id === 1 ? (
                            <a key={btn.id} href={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/regulations/${data.reglamento.id}/download`} className={`btn rounded-lg cursor-pointer flex size-12 place-items-center ${btn.id ===1? 'hover:bg-unimar/10' : (btn.id===2? 'hover:bg-gray-300/50': 'hover:bg-rose-300/50' )}`} onClick={(e) => e.stopPropagation()}>
                                <Image className='scale-110' src={btn.img} alt={btn.button} width={500} height={500} />
                            </a>
                        ) : (
                            <div key={btn.id} onClick={(e) => { e.stopPropagation(); 
